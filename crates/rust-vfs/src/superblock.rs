...
use crate::{Dentry, Inode, VFSResult};
...
pub struct SuperBlock {
    ...
    pub ops: SuperOperations,

    /// Inner mutable members
    pub inner: SpinLock<SuperBlockInner>,
}
...
pub struct SuperOperations {
    ...
    pub alloc_inode: Option<fn(&SuperBlock) -> Inode>,

    /// Marks the inode object dirty.
    pub dirty_inode: Option<fn(&Inode)>,

    /// Updates a filesystem inode with the contents of the inode object passed as the parameter.
    pub write_inode: Option<fn(&Inode, u32)>,

    /// Deletes an inode object, including the filesystem-specific data.
    pub delete_inode: Option<fn(&Inode)>,

    /// Updates a filesystem superblock with the contents of the object indicated.
    pub write_super: Option<fn(&SuperBlock)>,

    /// Invoked when flushing the filesystem to update filesystem-specific data structures on disk
    /// (used by journaling filesystems).
    pub sync_fs: Option<fn(&SuperBlock)>,

    /// Blocks changes to the filesystem and updates the superblock with the contents of the object indicated.
    pub lock_fs: Option<fn(&SuperBlock)>,

    /// Undoes the block of filesystem updates achieved by the lock_fs superblock method.
    pub unlock_fs: Option<fn(&SuperBlock)>,
}

#[derive(Debug)]
pub struct SuperBlockInner {
    /// Modification (dirty) flag, which specifies whether the superblock is dirty.
    pub dirty: bool,
}

impl SuperBlock {
    pub fn mount(name: &str, block_size: usize, max_size: usize, flags: MountFlags, ops: SuperOperations) -> Self {
        Self {
            block_size,
            max_size,
            ...
            ops,
            inner: SpinLock::new(SuperBlockInner { dirty: false }),
        }
    }
...
//     fn find_dentry(&self, path: &Path) -> Result<Arc<Dentry>, (Arc<Dentry>, usize)> {
